...
Factory Pattern: Controllers act as simple factories by creating new domain objects before saving them.

Where the Factory Pattern is used:
userController.ts — registerUser creates new userModel(...)
foodController.ts — addFood creates new foodModel(...)
orderController.ts — placeOrder creates new orderModel(...)
